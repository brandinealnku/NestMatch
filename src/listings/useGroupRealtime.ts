...
export function useGroupRealtime(groupId: string, refresh: () => void) {
  ...
  useEffect(() => {
    ...
    const channel = client.channel(`group-live:${groupId}:${user.id}`)
    ...
    return () => { window.clearInterval(poll); document.removeEventListener("visibilitychange", resume); void client.removeChannel(channel); };
