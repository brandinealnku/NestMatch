...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({ isConfigured: supabaseConfiguration.isConfigured, enabledProviders: authProviderConfiguration, isLoading, activeAction, authError, user, profile, refreshProfile,
    ...
    signInWithMagicLink: async email => { if (!supabase) throw new Error("Account sign-in is not configured."); if (activeAction === "magic-link") return; setActiveAction("magic-link"); setAuthError(""); const { error } = await requestMagicLink(supabase.auth, email); setActiveAction(null); if (error) { const message="We could not send a sign-in link. Please try again."; setAuthError(message); throw new Error(message); } },
